 ...
 * Scope: the islands listed in ISLANDS below. Never touches Kauaʻi pages (they live at /today/).
 *
 * `slug` is the ISLANDS.<slug> key and the data/<slug>.js filename; `path` is the URL directory.
 * They differ for Hawaiʻi Island: slug 'hawaii', public tree /big-island/ (/hawaii/ is reserved
 * for a state hub and must never be used for island pages).
 *
...
for (const isl of ISLANDS) {
  if (!existsSync(`data/${isl.slug}.js`)) continue;
  const beaches = arrText(readFileSync(`data/${isl.slug}.js`, 'utf8'), 'beaches');
  for (const b of beaches) {
    const f = `${isl.path}/today/${b.id}.html`;
    ...
    // 2b) coordinates — JSON-LD GeoCoordinates and the embedded var B must track the dataset.
    //     20 of 28 Big Island beaches shipped a pin that was not at the beach (up to 4.1 km out),
    //     so the SEO pages were publishing the wrong location in structured data too.
    for (const [key, val] of [['latitude', b.lat], ['longitude', b.lon]]) {
      const re = new RegExp('("' + key + '":\\s*)(-?[\\d.]+)');
      h = h.replace(re, (m, a, cur) => {
        if (Number(cur) === Number(val)) return m;
        notes.push(key + ' ' + cur + '\u2192' + val);
        return a + val;
      });
    }
    for (const [key, val] of [['lat', b.lat], ['lon', b.lon]]) {
      const re = new RegExp('("' + key + '":\\s*)(-?[\\d.]+)');
      h = h.replace(re, (m, a, cur) => (Number(cur) === Number(val) ? m : a + val));
    }
