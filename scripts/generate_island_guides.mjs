...
if (!['maui', 'oahu'].includes(island)) { console.error('usage: generate_island_guides.mjs <maui|oahu>'); process.exit(1); }
const LABEL = island === 'maui' ? 'Maui' : 'Oʻahu';
...
<p style="margin-top:8px"><a href="/${island}/today/">Beach conditions</a> &middot; <a href="/${island}-travel-guide">Travel guide</a> &middot; <a href="/${island}/guide/">${LABEL} guides</a> &middot; <a href="/${island}/today/dangerous-beaches">Dangerous beaches</a> &middot; <a href="/hawaii/">All islands</a> &middot; <a href="/?island=${island}">App</a></p></footer>
</div>
</body>
</html>
`;
...
mkdirSync(`${island}/guide`, { recursive: true });
const urls = [];
for (const p of PAGES) {
  const isPillar = p.set === 'pillar';
  const canonical = isPillar ? `${SITE}/${p.slug}` : `${SITE}/${island}/guide/${p.slug}`;
  const crumb = isPillar
    ? `<a href="/?island=${island}">Home</a> &rsaquo; ${stripTags(p.title).split(':')[0]}`
    : `<a href="/?island=${island}">Home</a> &rsaquo; <a href="/${island}/guide/">${LABEL} Guides</a> &rsaquo; ${stripTags(p.title).split(':')[0]}`;
  const crumbItems = isPillar
    ? [['Home', SITE + '/'], [stripTags(p.title).split(':')[0], canonical]]
    : [['Home', SITE + '/'], [`${LABEL} Guides`, `${SITE}/${island}/guide/`], [stripTags(p.title).split(':')[0], canonical]];
  const html = chrome(p.title, descOf(p), canonical, crumb, renderBody(p), jsonldFor(p, canonical, crumbItems));
  const out = isPillar ? `${p.slug}.html` : `${island}/guide/${p.slug}.html`;
...
const grps = SET_ORDER.map(set => {
  const rows = guides.filter(p => p.set === set).map(p => `<a class="row" href="/${island}/guide/${p.slug}"><span class="nm">${p.title}</span><span class="ar">→</span></a>`).join('');
...
const idxCanonical = `${SITE}/${island}/guide/`;
...
writeFileSync(`${island}/guide/index.html`, chrome(idxTitle, idxDesc, idxCanonical,
...
console.log('wrote', `${island}/guide/index.html`);

// sitemap append (idempotent)
const smFile = `sitemap-${island}.xml`;
if (existsSync(smFile)) {
  ...
  console.log(`sitemap-${island}.xml: +${added} URLs`);
}
